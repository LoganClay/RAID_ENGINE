...
KeyboardOut::KeyboardOut() {
	for (int i = 0; i < sizeof(this->lastFrameKeys); i++) {
		this->lastFrameKeys[i] = false;
	}

	for (int i = 0; i < sizeof(this->lastFrameButton); i++) {
		this->lastFrameButton[i] = false;
	}
}
...
void KeyboardOut::updateLastKey(bool keys[]) {
	for (int i = 0; i < sizeof(this->lastFrameKeys); i++) {
		this->lastFrameKeys[i] = keys[i];
	}
}

void KeyboardOut::updateLastButton(bool buttons[]) {
	for (int i = 0; i < sizeof(this->lastFrameButton); i++) {
		this->lastFrameButton[i] = buttons[i];
	}
}
